...
class TestAccessRouter:
    ...
    async def test_request_access_duplicate(self, client: AsyncClient):
        await client.post("/api/v1/access/request", json={"email": "dup@example.com", "name": "Dup"})
        r = await client.post("/api/v1/access/request", json={"email": "dup@example.com", "name": "Dup"})
        assert r.status_code in [200, 400, 409, 422]

    @pytest.mark.asyncio
    async def test_check_access_status(self, client: AsyncClient):
        response = await client.get("/api/v1/access/status?email=unknown@example.com")
...
class TestBlogRouter:
    ...
    async def test_get_blog_post_by_slug(self, client: AsyncClient):
        response = await client.get("/api/v1/blog/posts/nonexistent-slug")
        assert response.status_code in [200, 404]

    @pytest.mark.asyncio
    async def test_create_blog_post_unauthorized(self, client: AsyncClient):
        response = await client.post("/api/v1/blog/admin/posts", json={
...
class TestEmployeeRouter:
    ...
    async def test_get_subscribers(self, client: AsyncClient, auth_headers):
        response = await client.get("/api/v1/admin/subscribers", headers=auth_headers)
...
class TestSupportRouter:
    ...
    async def test_submit_ticket_unauthorized(self, client: AsyncClient):
        response = await client.post("/api/v1/support/ticket", json={
    ...
    async def test_submit_ticket(self, client: AsyncClient, auth_headers):
        response = await client.post("/api/v1/support/ticket", headers=auth_headers, json={
    ...
    async def test_get_tickets(self, client: AsyncClient, auth_headers):
        response = await client.get("/api/v1/support/tickets", headers=auth_headers)
        assert response.status_code in [200, 404]
