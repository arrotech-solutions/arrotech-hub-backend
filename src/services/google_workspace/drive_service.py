...
class DriveService:
    ...
    async def get_start_page_token(self) -> Dict[str, Any]:
        """
        Get the starting page token for listing changes in the drive.
        """
        try:
            service = self.base_client.get_service(self.service_name, self.version)
            ...
            logger.error(f"Error getting Google Drive start page token: {e}")
    ...
    async def watch_changes(
        ...
        """
        Watch for changes in Google Drive using the Changes API.
        
        Args:
            page_token: The token for the page of changes to start watching from.
            webhook_url: Your public API endpoint for notifications.
            channel_id: Unique string identifying this channel.
            token: Optional secret token sent in the X-Goog-Channel-Token header.
        """
        try:
            service = self.base_client.get_service(self.service_name, self.version)
            ...
            logger.error(f"Error creating Google Drive changes watch: {e}")
            return {'success': False, 'error': str(e)}

    async def list_changes(self, start_token: str) -> Dict[str, Any]:
        """
        List changes since the specified page token.
        """
        try:
            service = self.base_client.get_service(self.service_name, self.version)
            ...
            logger.error(f"Error listing Google Drive changes: {e}")
            return {'success': False, 'error': str(e)}
