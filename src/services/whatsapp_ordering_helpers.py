...
async def save_outgoing_agent_message(
    db: Any,
    user_id: Any,
    to_number: str,
    message_type: str,
    ...
    """Save outgoing messages (e.g. interactive cards/buttons) generated by the agent."""
    if not db or not user_id or not to_number:
        return
    try:
        from ..models import WhatsAppContact, WhatsAppMessage, WhatsAppMessageDirection, WhatsAppMessageStatus
        ...
        logger.warning(f"[WHATSAPP] Failed to save outgoing agent message to DB: {e}")
